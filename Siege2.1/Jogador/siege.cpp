...
{  char i;
   ...
   //--- g -------------
   for(j=1;j<=8;j++)
   {  NoPosicao *gprox, *gant, *f, *e, *gantant, *gproxprox, *gat, *h;
      no = grafo[id('g',j)];
...
{  int res;
   ...
   {  if(vez==PECA_VERMELHA)
      ...
      {  do
         ...
         realiza_movimento();
         if(flag_captura)
         {  pf("Continuar capturas (s/n) [padrão n]:");
            sf("%c%c",&c,&c);
         }
         else  c='n';
         if(c!='s')
      ...
      {  if(nova_jogada)   while(comunica->recebe()<=0);
         pf("%s\n",comunica->buf);
         ...
         pos_captura = grafo[id(letracap,nrocap)];
         realiza_movimento();
         while(comunica->recebe()<=0);
         if(strcmp(comunica->buf,"fim")==0)
         {  vez=!vez;
            nova_jogada = true;
         }
         else  nova_jogada = false;
      }      
   }
}

/*-------------------------------------------------------------------*/
void Siege::realiza_movimento()
{  Peca *peca;
   unsigned i;
