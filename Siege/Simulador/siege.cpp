...
{  int vez, nro, nrodes,i;
   ...
   {  if(vez==PECA_VERMELHA)
      ...
      {  flag_origem_valida = false;
         ...
         if(!flag_origem_valida || !flag_destino_valido)
         {  comunica[vez]->envia("Posição inválida");
            tela.imprime_colorido(AZUL, " => Posição inválida.\n");
         }
      } while(!flag_origem_valida || !flag_destino_valido);
...
{  char i, s[50];
   ...
   {  no = grafo[id('g',j)];
      ...
      no->peca = amarelas[(j-1)*2];

      no = grafo[id('f',j)];
      peca = new Peca(no);
      amarelas.push_back(peca);
      no->tipo = PECA_AMARELA;
      no->peca = amarelas[(j-1)*2+1];
   }
